...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Node2Vec baseline scaffold.")
...
def main() -> None:
    ...
    if config.get("dry_run", False) or not all(dependency_status.values()):
        notes = {
            "mode": "dry_run",
            "message": (
                "Node2Vec scaffold completed data validation and split generation. "
                "Install numpy/networkx and add a concrete embedding backend to continue."
            ),
        }
    ...
    raise NotImplementedError(
        "Training backend is intentionally left as the next step. "
