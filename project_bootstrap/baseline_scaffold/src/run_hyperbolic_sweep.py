...
from run_hyperbolic_baseline import run_hyperbolic_experiment


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run low-dim / curvature hyperbolic sweeps.")
...
def build_markdown_table(rows: list[dict]) -> str:
    if not rows:
        return "# hyperbolic sweep\n\nNo successful runs.\n"
    lines = [
        "# hyperbolic sweep",
        "",
        "| rank | run_id | variant | dim | curvature | val AP | test AP | test AUROC | best epoch |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    ...
    for variant in sweep_config["model_variants"]:
        for dim in sweep_config["dimensions"]:
            for curvature in sweep_config["curvatures"]:
                ...
                run_config["run_id"] = f"{base_config['run_id']}_{run_name}"
                run_config["artifacts_root"] = str((output_root / run_config["run_id"]).resolve().relative_to(workspace_root))

                print(
                    f"[sweep] running variant={variant} dim={dim} curvature={curvature} "
                    f"-> {run_config['artifacts_root']}"
                )
                try:
                    result = run_hyperbolic_experiment(run_config)
