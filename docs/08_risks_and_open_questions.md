...
| R03 | 数据快照、版本和 config 未锁定，导致结果不可复现 | High | Active | T10 生成 version manifest，T11 生成 data card |
...
8. `T02` review 模板标准化是否需要在后续治理阶段补回？
...
| D05 | `docs/review/` 模板标准化 | 当前 review 文件已可用，但模板尚未正式定稿；未发现 `T02_review.md` | 若后续 review 格式漂移，再补 T02 |
