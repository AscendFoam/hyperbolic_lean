...
| R30 | 论文 skeleton 贡献结构可能过宽，5 条 contributions 在 ITP/CPP 页数限制内难以充分展开 | Medium | Active | `T50_review` 已将该 warning 分类为 deferred；`docs/paper_outline.md` 当前保留 5 条 contributions 结构，但后续 paper drafting / venue shaping 应重新判断是否合并（如 C1+C5 合并为 pipeline+alignment），或把部分贡献降级为 appendix。`T54_review` 进一步提醒 abstract 已接近常见页数预算上界，说明正文压缩与贡献收束需要联动处理；`T59` 将作为这一风险的直接 submission-facing 收口任务。 |
