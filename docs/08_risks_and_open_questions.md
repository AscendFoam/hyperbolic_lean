...
1. `Field.Subfield`、`Order.Ring` 和更大的 `Mathlib.Algebra.Order` 中，哪一个最适合作为下一轮正式 benchmark 图？
2. 现有 `real_graphs_v1`、`hierarchy_focus_v1`、`mathlib_order_focus_v1` 诊断产物是否足以形成候选优先级，还是需要补跑缺失诊断？
