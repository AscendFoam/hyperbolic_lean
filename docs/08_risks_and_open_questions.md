...
| R02 | grouped retrieval 与 hop bucket 报告入口已通过 T12/T13 review 收口，但尚缺小配置端到端落盘 spot-check | Medium | Active | T14 执行 Milestone 1 smoke check；若无法运行，明确记录机器/依赖 blocker |
...
| R12 | T13 为保持最小范围引入了重复的 hop bucket flatten helper，未来维护时可能出现 runner 间字段漂移 | Low | Active | T14 可做轻量去重；若超出 smoke 范围，则延后到后续 refactor |

## 2. Open Questions

1. `hop_2 / hop_3 / hop_4_plus` 是否已经通过小配置端到端落盘 spot-check？
...
| D05 | 把所有 hop bucket flatten/format helper 完全抽到共享模块 | T13 review 只发现维护性重复，未影响 correctness；T14 可在轻量范围内处理，若牵涉较大则延后 | helper 字段出现漂移，或 T14 smoke check 需要改动相同逻辑 |
