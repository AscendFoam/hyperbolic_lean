...
1. T30 已形成 reviewed training alignment audit；T31A 已修复 grouped ancestor retrieval 的 query-level split completeness；T31 已在 grouped retrieval runner 中补入 reviewed 最小 query-grouped loss；T32/T33 已分别完成 matched GCN/HGCN grouped 5-seed 对照；T34 已把 grouped-vs-binary 与 matched GCN-vs-HGCN 结论收口成正式总结；T40 已把 provenance split 的配置与输出位置冻结成可复用协议入口；T41 已把 frozen config 真正落成六个 split 图目录并接通 diagnostics pipeline；T42 已在 provenance-aware split 上完成受约束的 GCN/HGCN 5-seed sweeps；T43 已把 provenance 结构诊断、seed sweep 结果、风险状态与项目叙事统一收口，并将 Milestone 4 结论固定为 provenance-conditional；T50 已把这些 reviewed artifact 提炼成 paper skeleton；T51 已从 proof-side bridge 中正式选定 ancestor explanation 作为 MVP；T52 已把该 MVP 收敛成 reviewed 的最小 demo implementation package；`T52a` 已实现并通过 review，把 proof-side bridge 落地为真实 demo CLI 与 demo report；`T53` 已完成 milestone review 并通过 review，正式将项目裁决为 **Narrow**；`T54`~`T60` 已依次把 paper-facing draft、precision cleanup、figure/table source rendering、artifact packaging、final paper editing 与 submission-facing 收尾收口；`T61` 已通过 review 并完成 final repo packaging / handoff freeze；`T62` 已通过 review 并完成 venue-specific planning：`docs/venue_submission_plan.md` 已固定 ITP 为主 venue 路径、CPP 为 co-primary，并列出剩余格式化与提交差额。`T63` 已完成 ITP-targeted LaTeX conversion / core figure rendering；`T63_review` 额外把 figure visual QA 留给 `T64`，其核心是 `paper/itp/figures/F1_provenance_structure.png` 与 `F2_hop_depth_delta.png` 的最后可读性收口。
2. relation provenance split 已从”已冻结协议”推进到”已真实生成图并经结构诊断验证”的正式实验阶段；paper-facing 数值精度问题 `R28`/`R29` 已由 `T56` cleanup 关闭/修正；figure/table source rendering 已由 `T57` 收口；artifact packaging 已由 `T58` 收口；final paper editing 已由 `T59` 收口；submission-facing 收尾已由 `T60` 收口；repo package boundary 与 handoff-facing 辅助文档角色已由 `T61` review PASS 正式冻结；venue 路径已由 `T62` review PASS 正式冻结。当前缺的不是再做 venue choice 或 LaTeX 转换，而是先把 F1/F2 的视觉 QA 收口，再进入最终 submission bundle assembly（author info、CFP 确认、打包压缩）。
3. proof-side utility 已完成 MVP 选型、demo implementation 与 milestone 级裁决，当前缺的不是继续扩写 proof-side 功能面，也不是继续做 LaTeX 转换，而是最终 submission bundle assembly 与打包。
