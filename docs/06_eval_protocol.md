...
>
> 状态：实验与协议口径已由 T12/T13/T14/T31A/T31/T32/T33/T41/T42/T43 收口；T50~T63 已把这些 reviewed 证据推进到 paper-facing / submission-facing 资产。当前阶段是 core figure QA / regeneration，不涉及新的评测语义变更。
...
| proof-side utility | 连接 proving workflow | ancestor explanation CLI demo 已完成，可作为 paper-facing bridge |
...
- `T41` 已通过 adversarial review，六个 provenance split 图已真实生成并完成边数与 identity 校验；`T42` 已通过 adversarial review，provenance-aware GCN/HGCN grouped seed sweeps 已完成，并确认 `explicit_only` 为 primary evidence、`synthesized_only` 为 controlled diagnostic、`hierarchy_mixed` 为 reproducibility check；`T43` 已通过 milestone review，Milestone 4 的正式结论固定为 provenance-conditional；`T50` 已通过 review 并产出 `docs/paper_outline.md`；`T51` 已通过 review 并正式选定 ancestor explanation 作为 proof-side MVP；`T52` 已通过 review 并把该 MVP 收敛成唯一的最小 demo implementation package；`T52a` 已通过 adversarial review，ancestor explanation demo CLI 与 demo report 已真实落地；`T53` 已通过 review，并把项目状态正式裁定为 **Narrow**；`T54` 已通过 review 并产出可继续修订的 paper-facing draft 首版；`T55` 已通过 review 并完成第二轮文稿结构 refinement；`T56` 已通过 review 并完成 precision cleanup：(1) `R29` 已修正——provenance_summary.md Section 5.1 表格中 FS GCN synthesized_only MAP 已校正为 verified T42 value `1.0000 ± 0.0000`；(2) `R28` 已解析并关闭——经 T42 artifact 审计确认为 metric naming confusion（`test_average_precision` vs `grouped_test_map`），两条指标均计算正确、内部一致，不存在数据管线 bug；(3) `paper_draft.md` 与治理文档已同步。`T57` 已通过 review，并完成 figure/table source rendering：`docs/paper_figures_and_tables.md` 已成为 publication-facing 图表源文档。`T58` 已通过 review，并完成 artifact packaging：`docs/paper_artifact_package.md` 已建立 submission-facing source-to-claim 映射、table/figure-to-source 映射、已知排除项、活跃风险边界和提交检查清单；`T58_review` 的两个 non-blocking notes（core-table 术语统一、Table T1 HGCN source mapping 精度说明）已并入 `T59`。`T59` 已通过 review，并完成 final paper editing / venue shaping：贡献结构保持 5 条（C1–C5）不变并加入 page-budget-aware 措辞；artifact package 中 core-table 术语已统一为"4 core tables + 1 summary table"；Table T1 HGCN source mapping 已改为"T33 primary，T42 cross-check"。`T59_review` 的两个 non-blocking notes（`R30 page budget check` checklist 勾选同步、Page Budget Note 自洽性补强）已并入 `T60`。`T60` 已通过 review，并完成 submission-facing 收尾：`paper_artifact_package.md` 的 `R30 page budget check` 勾选状态已与 T59 决策同步；`paper_outline.md` 的 Page Budget Note 已补强自洽说明；`paper_draft.md` Section 7.4 已同步更新。`T60_review` 的两个 non-blocking notes 已收口。`T61` 已通过 review，并完成 final repo packaging / handoff freeze：`paper_artifact_package.md` 新增 Section 9 明确 repo package boundary，`docs/for_human/T60_review_explanation.md` 与 `docs/worker_summary/T60_worker_summary.md` 已标注为 explanatory handoff-facing aids。`T62` 已通过 review，并完成 venue-specific formatting / submission planning：`docs/venue_submission_plan.md` 已固定 ITP 为主 venue 路径、CPP 为 co-primary，并列出剩余格式化与提交差额。`T63` 已通过 review 并完成 ITP-targeted LaTeX conversion / core figure rendering；但 `T63_review` 将 F1/F2 的视觉收口留给 `T64`，当前阶段已切换到 `T64`：core figure QA / regeneration。`R25`（clean-environment reproducibility）、`R30`（contributions 页数预算）与 `R34`（模板/编译链与最终 bundle 的 scope diffusion）继续保留。
